...
![操作系统是什么](../../../../tobebetterjavaer/images/sidebar/sanfene/os-be55aec1-e7ab-433f-97f1-14d99960b6bf.png)
...
![操作系统主要功能](../../../../tobebetterjavaer/images/sidebar/sanfene/os-eee82952-c96f-45c9-835e-29db37c0f6d8.png)
...
![用户态&内核态切换](../../../../tobebetterjavaer/images/sidebar/sanfene/os-b358cdae-18b6-45d4-8a5b-4ea3a7cfc273.png)
...
![并发和并行](../../../../tobebetterjavaer/images/sidebar/sanfene/os-fb7891d8-8330-494b-9bc1-cf829b5cc82d.png)
...
![进程上下文切换-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-187d1cf9-971d-4395-b888-5e6eaf2be5f1.png)
...
![进程3种状态](../../../../tobebetterjavaer/images/sidebar/sanfene/os-5df30631-ad7d-4c65-af20-50b7b615eca8.png)
...
![进程5种状态](../../../../tobebetterjavaer/images/sidebar/sanfene/os-ae17a9dc-f555-481a-ba4a-caca06120be7.png)
...
![进程调度算法](../../../../tobebetterjavaer/images/sidebar/sanfene/os-18911b0a-7326-467f-94fb-8c01a6980578.png)
...
![先来先服务](../../../../tobebetterjavaer/images/sidebar/sanfene/os-93088d03-80c9-46c5-9eaf-eead2adb6e12.png)

- **短作业优先**

非抢占式的调度算法，按估计运行时间最短的顺序进行调度。长作业有可能会饿死，处于一直等待短作业执行完毕的状态。因为如果一直有短作业到来，那么长作业永远得不到调度。

![短作业优先](../../../../tobebetterjavaer/images/sidebar/sanfene/os-517e8392-64fe-4de3-9e1c-b3a944822aba.png)

- **优先级调度**

为每个进程分配一个优先级，按优先级进行调度。为了防止低优先级的进程永远等不到调度，可以随着时间的推移增加等待进程的优先级。

![优先级调度](../../../../tobebetterjavaer/images/sidebar/sanfene/os-7c4441cf-7b8c-4660-8ba8-29b8076e2da1.png)
...
![时间片轮转](../../../../tobebetterjavaer/images/sidebar/sanfene/os-ad224c3a-8ac9-4230-84e4-ec434d5b49f9.png)
...
![进程间通信方式](../../../../tobebetterjavaer/images/sidebar/sanfene/os-34b8635f-ec27-46c6-87c3-91440fea662d.png)
...
- 管道：管道可以理解成不同进程之间的对白，一方发声，一方接收，声音的介质可是是空气或者电缆，进程之间就可以通过管道，**所谓的管道就是内核中的一串缓存**，从管道的一端写入数据，就是缓存在了内核里，另一端读取，也是从内核中读取这段数据。
  ...
  ![“奉先我儿”](../../../../tobebetterjavaer/images/sidebar/sanfene/os-5994e202-0d59-4a86-8f79-a17a5d0bd3d3.png)
...
![消息队列](../../../../tobebetterjavaer/images/sidebar/sanfene/os-d3d8bef5-ae32-487f-828d-aaddb93867c4.png)
...
![共享内存](../../../../tobebetterjavaer/images/sidebar/sanfene/os-d9e3cfaf-01e7-42ff-9290-94ef4a5c7d5e.png)
...
- 信号量：信号量我们可以理解成红绿灯，红灯行，绿灯停。**它本质上是一个整数计数器**，可以用来控制多个进程对共享资源的访问。它常作为一种锁机制，防止某进程正在访问共享资源时，其他进程也访问该资源。因此，主要作为进程间以及同一进程内不同线程之间的同步手段。
  ...
  ![信号量](../../../../tobebetterjavaer/images/sidebar/sanfene/os-5fb765af-918c-4037-a3ad-4cad4d985e16.png)
...
![多线程-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-271e450b-66ef-4f6c-b823-8e0b73347825.png)

线程与进程的⽐较如下：

- 调度：**进程是资源（包括内存、打开的⽂件等）分配的基本单位**，**线程是 CPU 调度的基本单位**；

  ```
  1、即时间片时直接分配给线程的，还不是给进程，再由进程分配给线程
  2、一个进程至少有一个线程，一个进程可以运行多个线程
  ```
...
![内核态线程实现](../../../../tobebetterjavaer/images/sidebar/sanfene/os-30b84285-8027-4720-b50b-3b0fb18c756f.png)

- **⽤户态线程实现**：在⽤户空间实现线程，不需要内核的参与，内核对线程无感知。

![用户态线程](../../../../tobebetterjavaer/images/sidebar/sanfene/os-57886181-56fe-42bf-85e1-4d062455788a.png)
...
![混合线程实现](../../../../tobebetterjavaer/images/sidebar/sanfene/os-1597d159-1b07-48ae-ac86-7e9b9cb85876.png)
...
![临界区互斥-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-48a5b48b-2474-4460-ac5b-9502883b353f.png)
...
![加锁和解锁-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-4bd37c12-992d-4660-8b63-15c3b35d105f.png)
...
![死锁](../../../../tobebetterjavaer/images/sidebar/sanfene/os-e0069c37-d758-4df0-a2fd-3722ec93c61a.png)
...
```
活锁是特殊的饥饿锁，表现在一直是活的，一直尝试-失败-尝试-失败
```
...
![虚拟内存](../../../../tobebetterjavaer/images/sidebar/sanfene/os-ec171cea-0046-4709-a390-7babf3272c49.png)

```
虚拟内存技术将不同物理内存映射在一块，使得应用程序认为其拥有一块连续的内存。并且在内存不足的时候，操作系统会自动调用物理内存映射到虚拟内存中充当内存。
```
...
![虚拟地址、段表、物理地址](../../../../tobebetterjavaer/images/sidebar/sanfene/os-075df152-7b77-40c7-abdb-1aa0280d958b.png)

我们来看一个映射，虚拟地址：段3、段偏移量500  ---->  段基地址7000+段偏移量500 ----> 物理地址：7500。

![段虚拟地址映射](../../../../tobebetterjavaer/images/sidebar/sanfene/os-a57baf1c-9612-49dd-8b23-8b00a0c63cef.png)
...
![内存分页](../../../../tobebetterjavaer/images/sidebar/sanfene/os-4cdd5179-4b88-4aa6-b9c2-9ef8fdc745dc.png)
...
![多级页表示意图](../../../../tobebetterjavaer/images/sidebar/sanfene/os-3021f22f-b9a3-49d9-9e80-6d3abaf5a61a.png)
...
![TLB示意图-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-cdc02a2f-59bf-45dc-8531-83b46f77bd65.png)
...
![常见页面置换算法](../../../../tobebetterjavaer/images/sidebar/sanfene/os-6effefb6-67d2-4155-a3fc-4b27a319391a.png)
...
![按照进入内存早晚构建的页面链表 ](../../../../tobebetterjavaer/images/sidebar/sanfene/os-8cccc78d-ba25-4c0a-8ee8-0913e80af7b7.png)
...
![LRU实现](../../../../tobebetterjavaer/images/sidebar/sanfene/os-90810f7f-aa5b-4626-9761-c2c622b5e561.png)
...
![时钟页面置换算法](../../../../tobebetterjavaer/images/sidebar/sanfene/os-3646408f-999e-48a1-84e9-113525778aca.png)
...
![硬链接-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-d3f778f9-506b-4b93-9fb7-40eb0a79874e.png)
...
- 软链接相当于重新创建⼀个⽂件，这个⽂件有**独⽴的** **inode**，但是这个**⽂件的内容是另外⼀个⽂件的路径**，所以访问软链接的时候，实际上相当于访问到了另外⼀个⽂件，所以**软链接是可以跨⽂件系统的**，甚⾄**⽬标⽂件被删除了，链接⽂件还是在的，只不过打不开指向的文件了而已。**
  ...
  ![软链接-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-81abf13c-5c60-4263-8fcb-c79c33d865e8.png)
  
  ```
  具体区别：
  1、创建方式不同：软连接 ln -s src.txt now.txt   硬链接： ln src.txt now.txt
  2、软连接有新的inode号，硬链接没有
  3、删除影响不同：软连接相当于创建源文件的快捷方式，删除不影响源文件。
  				硬链接相当于创建文件的另一个入口（引用），和源文件inode一样，也就是说只有当所有相同的inode文件删除了，文件才真正删除。
  4、作用域不同：软连接可以是文件、目录还可以是不同文件系统下的文件，硬链接只能是本系统下的文件。因为软连接文件存储的是源文件的路径地址。
  ```
...
![传统文件传输示意图-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-1e595664-6585-4d56-8939-08b7ce510218.png)
...
mmap() 系统调⽤函数会直接把内核缓冲区⾥的数据「**映射**」到⽤户空间，这样，操作系统内核与⽤户空间就不需要再进⾏任何的数据拷⻉操作。（windows、linux）

![mmap示意图-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-b151f8198618367ace3883ef55c63edeb21ce58b.png)



1. 首先向操作系统发送一个mmap命令(上图1.1)，此时由**用户态**切换为**内核态**。
2. 系统收到mmap命令后，会用DMA把数据从磁盘读到到内存的缓存区中(上图1.2)。
3. 返回数据缓存区的一个映射mmap（上图1.3），由**内核态**切换到**用户态**。
4. 用户在发起一个write的操作（上图1.4），由**用户态**切换到**内核态**。
5. cpu再把内存缓存区中的数据拷贝到另一个缓存区上(上图1.5)
6. DMA把数据拷贝到网卡上后返回(上图1.6)，并由**内核态**切换到**用户态**

mmap的操作共有**4个上下文的切换和3次的数据拷贝过程**，比普通的拷贝少了一次cpu的拷贝。
...
![mmap示意图-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-c83d70cf3bc79f3d12fb70a03b14781b738b294c.png)
...
1. 用户往操作系统发直sendfile的命令，此时由**用户态**切换到**内核态**
2. 系统收到命令后，由DMA把数据从磁盘上读取到系统缓存区上
3. 然后由cpu把读到的到数据拷贝到另一个缓存区上
4. 再由DMA把数据写到网上上
5. 然后返回，由**内核态**切换到**用户态**

sendfile的过程由**2次上下文的切换和3次的数据拷贝过程**，整个过程读到的数据**对用户空间不可见**，适用静态文件服务器。（如nginx静态资源）
...
```
DMA: （Direct Memory Access）直接内存访问技术，通过它来进行内存和IO设备的数据传输。大至就是CPU给DMA发一个指令，然后DMA开始干活，然后CPU干别的活，DMA干完后告诉CPU，从而减少了CPU的等待时间
```
...
![阻塞I/O](../../../../tobebetterjavaer/images/sidebar/sanfene/os-f06db5ff-661c-4ddf-9115-4ed9c9a21d01.png)

- **非阻塞I/O**

⾮阻塞的 read 请求在数据未准备好的情况下⽴即返回，可以继续往下执⾏，此时应⽤程序不断轮询内核，直到数据准备好，内核将数据拷⻉到应⽤程序缓冲区， read 调⽤才可以获取到结果。

![非阻塞I/O](../../../../tobebetterjavaer/images/sidebar/sanfene/os-771e014e-7ed9-4101-8bb5-4413b8069fd6.png)
...
![基于非阻塞的I/O多路复用](../../../../tobebetterjavaer/images/sidebar/sanfene/os-86e54fa3-ad36-43c7-9d2d-5a68139c310f.png)
...
![异步/IO](../../../../tobebetterjavaer/images/sidebar/sanfene/os-869021ed-5e4e-4490-9174-7291d8ddf55c.png)
...
基于⾮阻塞的 I/O 多路复⽤好⽐，老三发现UP没更，就去干别的，过了一会儿B站推送消息了，老三打开浏览器等待缓存，缓存好后可以看。

异步I/O就是，老三说UP你该更了，UP赶紧爆肝把视频做出来，并将视频缓存到本地，然后通知老三，一收到通知立马能看。
...
![I/O多路复用](../../../../tobebetterjavaer/images/sidebar/sanfene/os-9b276b14-eb1b-47bf-b2aa-25212e1bbdf8.png)
...
![epoll接口作用-来源参考[3]](../../../../tobebetterjavaer/images/sidebar/sanfene/os-cca76ac4-cfb4-4374-8fc6-256cd4d3893f.png)
