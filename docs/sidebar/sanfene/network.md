...
- URI，统一资源标识符(Uniform Resource Identifier， URI)，**标识**的是 Web 上每一种可用的资源，如 HTML 文档、图像、视频片段、程序等都是由一个 URI 进行标识的。
...
注意：ACK报文段可以携带数据，单如果不携带数据则不消耗序号

最后一次握手在默认不携带数据的情况下， 由于SYN 不是 1 ， 是不消耗序列号的。 所以三次握手结束后， **客户端下一个发送的报文中** seq 依旧是 x+1， 示意图如下

![大白话三次握手](../../../images/nice-article/tcp-20200727230757613.gif)
...
- 防止已失效的连接请求报文段突然又传送到了服务端，因而产生错误。（不严谨，更好的说法是为了确定服务端发送消息和客户端接受消息的能力，如果没有第三次握手，服务端是不能先一步发送消息的，因为不能确定二次握手客户端是否已经收到，且明确了seq，而这于tcp全双工的概念违背，全双工并没有定义消息先发送的一方是谁）
...
如果服务器有处于 TIME-WAIT 状态的 TCP，则说明是由**服务器⽅主动发起的断开请求**。
...
网络层协议负责提供**主机间**（ip）的逻辑通信；传输层协议负责提供**进程间**（端口）的逻辑通信。

### 49.IP 地址有哪些分类？

一个 IP 地址在这个互联网范围内是惟一的，一般可以这么认为，IP 地址 = {<网络号>，<主机号>}。
...
13台根服务器：美11+瑞典1+日本1， 但是IPV6中国有了四台
...
![网络包封装流程](../../../images/nice-article/net-20180316141535710-1359862908.png)